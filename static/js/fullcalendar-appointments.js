$(document).ready(function () {
  ...
  $(".calendar-save-button").on("click", function (e) {
    ...
    if (
      $(".time-input").text().length === 0 ||
      $(".validation-time").text().length === 0 ||
      $(".validation-date").val().length === 0
    ) {
      $("#date-alert-2").show();
    }
    ...
    if (
      $(".validation-date").val().length > 0 &&
      $(".validation-time").text().length > 0 &&
      $(".validation-interval").text().length > 0
    ) {
      $("#date-alert-2").hide();
    }
    if ($(".validation-reserv").text().length > 0) {
    ...
    if (
      $("#name_input").val().length > 0 &&
      $(".validation-date").val().length > 0 &&
      $(".validation-time").text().length > 0 &&
      $(".validation-interval").text().length > 0 &&
      $(".validation-reserv").text().length > 0
    ) {
  ...
  $("#calendar-ms").fullCalendar({
    ...
    select: function (start, end) {
      ...
      $(".modal")
        .find(".calendar-modal .icon-content")
      ...
      $(".modal")
        .find(".calendar-modal #whole_day_checkbox")
        .prop("checked", false);
  ...
  $(".calendar-modal #modal_2_save").on("click", function (event) {
    ...
    date2 = moment(
      $(".calendar-modal #modal_2_end").val(),
      "DD/MM/YYYY"
    ).format("MM/DD/YYYY");
    a1 = date1.split("/");
    a2 = date2.split("/");
  ...
  $(".calendar-save-button").on("click", function () {
    ...
    start_date = new Date(
      beginingTime[2] +
        ...
        beginingTime[0] +
        " " +
        $(".beginChoices #time_input").text().trim() +
        ":00"
    );
        ...
        if (
          ...
          $("#common_contacts").append(
            '<div id='+ contacts[key].id +' class="card mt-1 p-2 auto_name " style="user-select:none">' +
              ...
              "</div>" +
              '<div class="pl-3"> ' +
              " <h6>"+ contacts[key].first_name +" " + contacts[key].last_name +"</h6> " +
              '<span class="blockquote-footer">'+ contacts[key].phone +' </span> ' +
              "</div>" +
              "</div> " +
              "</div>"
          );

    ...
    .then((json) => {
        ...
        json.forEach(element => {
          if(element.folder === "Inbox") {
            inbox.push(element)
            $("#mailContent").append('<li  num='+ element.num +' id="'+ element.id +'" class="list-group-item d-flex align-items-center mailListGroup"><div class="d-flex h-100 w-100 align-items-center justify-content-between"><span class="d-flex align-items-center"><input type="checkbox" class="checkBoxMails" name="'+ element.folder +'" id="mailCheckboxes"><span class="emailNameTitle">' + element.sender +'</span><span class="emailSubject">' + element.subject +'</span></span></li>');
            isDataCame = true;
            $("#preLoader").css("display","none");
        ...
        $(".deleteMessage").click(function() {
          ...
          json.forEach(element => {
            if(element.folder === "Trash") {
              $("#mailContent").append('<li num='+ element.num +' id="'+ element.id +'" class="list-group-item d-flex align-items-center mailListGroup"><div class="d-flex h-100 w-100 align-items-center justify-content-between"><span class="d-flex align-items-center"><input type="checkbox" class="checkBoxMails" name="'+ element.folder +'" id="mailCheckboxes"><span class="emailNameTitle">' + element.sender +'</span><span class="emailSubject">' + element.subject +'</span></span></li>');
              $(".mailListGroup").click(function(){
                $(".mailSubject").html('<div class="spinner-grow text-dark" role="status"><span class="sr-only">Loading...</span></div>');
                $(".mailSender").html('<div class="spinner-grow spinner-grow-sm text-dark" role="status"><span class="sr-only">Loading...</span></div>')
                ...
                json.forEach(element => {
                  if(element.id == mail) {
                    ...
                    let mailSender = element.sender.split('');
                    let color = ["purple","blue","green","maroon","darkorange"];
                    $(".emailImage").html(mailSender[0].toUpperCase() + '' + mailSender[1].toUpperCase());
                    let rNumber = Math.floor(Math.random()*5);
